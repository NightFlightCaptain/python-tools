...
def mask_change(source_img_path,add_img_path,save_path,rgb):
    ...
    # 将黑白化的add图片和source图片取并集（允许add添加区域超过source中已经涂色的区域并覆盖），取出需要的部分为白色，其余地方为黑色
    union_mask = cv2.bitwise_or(source_mask_tem, add_mask_tem)

    # 取出需要的部分为黑色，其余地方为白色
    union_mask = 255 - union_mask
    source_mask_effective = cv2.bitwise_and(source_image, source_image, mask=union_mask)
    # save_path_2 = save_path.replace('.bmp', 'Two.png')
    # cv2.imwrite(save_path_2, add_mask)

    # 取出add图片中真正有效的涂色区域
    add_mask_effective = cv2.bitwise_and(add_image, add_image, mask=add_mask_tem)

    # 有效涂色区域和source文件叠加
    final_image = add_mask_effective + source_mask_effective
